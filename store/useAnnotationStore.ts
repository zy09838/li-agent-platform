...
import type { AnnotationAppState, Annotation, ToolType, ImageMetadata, ImageFile, SegmentationMask, ClipboardData } from '../types/annotation';
...
interface AnnotationStore extends AnnotationAppState {
    // 主题
    theme: Theme;
    ...
    addImagesFromUrls: (images: Array<{ id: string; name: string; url: string; width: number; height: number; annotations?: Annotation[]; segmentationMasks?: SegmentationMask[] }>) => void;
    clearImages: () => void;
...
export const useAnnotationStore = create<AnnotationStore>((set, get) => ({
    ...
    deleteSelectedAnnotations: () => {
        const { selectedAnnotationIds, currentImageId } = get();
    ...
    addImagesFromUrls: (images) => {
        const newImages: ImageFile[] = images.map(img => ({
            id: img.id || uuidv4(),
            name: img.name,
            url: img.url,
