...
import type { AppState, Annotation, ToolType, ImageMetadata, ImageFile, SegmentationMask, ClipboardData } from './annotationTypes';
...
// 预设缺陷类型模板
const DEFAULT_DEFECT_TEMPLATES: DefectTemplate[] = [
...
// 从 localStorage 加载自定义缺陷类型
const loadCustomDefectTemplates = (): DefectTemplate[] => {
    try {
        const stored = localStorage.getItem('customDefectTemplates');
        if (stored) {
            return JSON.parse(stored);
        }
    } catch (e) {
        console.error('Failed to load custom defect templates:', e);
    }
    return [];
};

// 保存自定义缺陷类型到 localStorage
const saveCustomDefectTemplates = (templates: DefectTemplate[]) => {
    try {
        localStorage.setItem('customDefectTemplates', JSON.stringify(templates));
    } catch (e) {
        console.error('Failed to save custom defect templates:', e);
    }
};

// 合并预设和自定义缺陷类型
export let DEFECT_TEMPLATES: DefectTemplate[] = [...DEFAULT_DEFECT_TEMPLATES, ...loadCustomDefectTemplates()];

// 添加自定义缺陷类型
export const addCustomDefectTemplate = (label: string, color: string): boolean => {
    // 检查是否已存在
    if (DEFECT_TEMPLATES.some(t => t.label === label)) {
        return false;
    }
    const newTemplate: DefectTemplate = { label, color, isCustom: true };
    const customTemplates = loadCustomDefectTemplates();
    customTemplates.push(newTemplate);
    saveCustomDefectTemplates(customTemplates);
    DEFECT_TEMPLATES = [...DEFAULT_DEFECT_TEMPLATES, ...customTemplates];
    return true;
};

// 删除自定义缺陷类型
export const removeCustomDefectTemplate = (label: string): boolean => {
    const customTemplates = loadCustomDefectTemplates();
    const index = customTemplates.findIndex(t => t.label === label);
...
// 获取所有缺陷类型（用于组件刷新）
export const getDefectTemplates = (): DefectTemplate[] => {
    DEFECT_TEMPLATES = [...DEFAULT_DEFECT_TEMPLATES, ...loadCustomDefectTemplates()];
...
export const useAnnotationStore = create<AppStore>((set, get) => ({
    images: [],
    currentImageId: null,
    selectedTool: 'select',
